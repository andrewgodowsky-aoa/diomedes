...
/**
 * A model reviewer's decision, said as what it is. The heading never uses a
 * person's name or the word approved on its own, because this record must not
 * be mistakable for the person having seen the output.
 */
export function ReviewerRecord({ need }: { need: Need }) {
  const reviews = need.reviews ?? [];
  if (!reviews.length) return null;
  const latest = reviews[reviews.length - 1];
  const used = need.authorization?.reviewId === latest.id;
  const outcome =
    latest.decision === 'approve'
      ? used
        ? 'Approved by a separate model reviewer. You did not review this output.'
        : 'A separate model reviewer approved this, and it was not applied under that approval.'
      : latest.decision === 'require-human'
        ? 'A separate model reviewer asked you to decide.'
        : latest.decision === 'reject'
          ? 'A separate model reviewer recommended a change. Nothing was applied and you can still decide.'
          : 'A separate model reviewer could not decide. Nothing was applied and it is waiting for you.';
  return (
    <details className="approval-record" aria-label="Reviewer record">
      <summary className="approval-summary">Reviewer record</summary>
      <p>{outcome}</p>
      {latest.note && <p className="approval-recorded">{latest.note}</p>}
      <dl className="facts approval-facts">
        <dt>Decision source</dt>
        <dd>model-reviewer</dd>
        <dt>Decision</dt>
        <dd>
          {latest.decision} ({latest.reasonCode})
        </dd>
        <dt>Reviewer model</dt>
        <dd className="attribution-wrap">
          {latest.modelSource === 'runtime'
            ? latest.reportedModel
            : 'not reported by the runtime'}
          {latest.requestedModel ? ` · requested ${latest.requestedModel}` : ' · no model requested'}
        </dd>
        <dt>Reviewer engine</dt>
        <dd className="attribution-wrap">{latest.engine}</dd>
        <dt>Reviewer run</dt>
        <dd className="attribution-wrap">
          {latest.invocationId}
        ...
        <dt>Independence</dt>
        <dd>{latest.independence}</dd>
        <dt>Worker model</dt>
        <dd className="attribution-wrap">
          {latest.proposerReportedModel ?? 'not reported by the runtime'}
        </dd>
        <dt>Policy</dt>
        <dd className="attribution-wrap">
          {latest.policy.version} · {latest.policy.result}
        </dd>
        <dt>Proposal</dt>
        <dd>{latest.proposalDigest}</dd>
        <dt>Action</dt>
        <dd>{latest.actionDigest}</dd>
        <dt>Decided</dt>
        <dd>{new Date(latest.decidedAt).toLocaleString()}</dd>
      ...
      return (
        <div className={`approval-status is-${state}`} aria-label="Approval record">
          ...
          <details className="approval-record">
            ...
            {/* The reviewer's own record sits beside this one and carries every
                reviewer fact, so this paragraph says only where authority came
                from. Saying it twice would read as two separate assurances. */}
            <p>
              {grant.reviewId
                ? 'Matched the task scope. Authorization came from the scope you confirmed.'
                : 'Matched the task scope. Authorization came from the confirmed scope; this output did not receive an individual approval.'}
            </p>
            <dl className="facts approval-facts">
              <dt>Authorization</dt>
              <dd>{grant.id}</dd>
              ...
              <dt>Reviewer</dt>
              <dd className="attribution-wrap">{grant.reviewId ?? 'none, deterministic scope'}</dd>
            </dl>
          </details>
          <ReviewerRecord need={need} />
        </div>
      );
    }
    return (
