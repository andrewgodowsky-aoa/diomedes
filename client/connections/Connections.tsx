...
export function Connections({ projectId }: { projectId: string }) {
  ...
  return <div className="connections">
    <div className="bhead"><h1>Connections</h1><span className="mono">Experimental / synthetic data</span></div>
    <div className="connection-desktop">
      <section className="cblock">
        <p>Bring menu availability into a scoped investigation, a manager task and a record you can inspect.</p>
        <p className="quiet">This local demonstration uses three fictional restaurants. Live Toast access is unavailable. Monitoring runs while this app is open.</p>
        {error && <p role="alert" className="cerr">{error}</p>}
      </section>
      {!connection && <section className="cblock">
        ...
        <div className="cacts"><button className="go" disabled={busy} onClick={() => void perform(async () => setPlan(await post('propose', {
          text, ...(threshold === '' ? {} : { threshold: Number(threshold) }),
          ...(start && end ? { serviceWindow: { start, end, timeZone: zone, days: [0, 1, 2, 3, 4, 5, 6] } } : {}),
        })))}>Prepare connection proposal</button></div>
      </section>}
      {plan && !connection && <section className="cblock" aria-label="Connection proposal">
        ...
        <p className="quiet">IN_STOCK means available; its quantity is not tracked. These observations do not measure ingredient inventory. Ordering and other Toast writes are unavailable.</p>
        {plan.plan.questions.length > 0 ? <ul className="crules">{plan.plan.questions.map((question) => <li key={question}>{question}</li>)}</ul>
          : <div className="cacts"><button className="go" disabled={busy} onClick={() => void perform(async () => { await post('adopt', { id: plan.plan.id, digest: plan.digest }); setPlan(null); })}>Approve synthetic connection</button></div>}
      </section>}
      {connection && <>
        <section className="cblock">
          <h2>{connection.connection.name}<span className="mono" data-testid="connection-health">{connection.health}</span></h2>
          <p>Scope: {connection.connection.resources?.map((item) => item.name).join(', ') ?? 'Review session authority to inspect saved evidence.'}</p>
          <p className="quiet">Last full refresh: {connection.connection.lastReconciledAt ?? 'No current refresh'}. Source gaps require reconciliation.</p>
          <div className="cacts">
            {(!data?.authorized || connection.health !== 'healthy' && connection.health !== 'stale') && <button className="go" disabled={busy} onClick={() => void perform(() => post('resume'))}>Enable synthetic connection for this session</button>}
        ...
        {data?.observations && <section className="cblock"><h2>Reported availability</h2><div className="ctable"><table><thead><tr><th>Location</th><th>Menu item</th><th>Availability</th><th>Quantity</th><th>Received</th></tr></thead><tbody>
        ...
        {data?.authorized && <section className="cblock"><h2>Signed event demonstration</h2><p>A synthetic source reports quantity 3 at Downtown. The active threshold and service hours decide whether it creates one manager task.</p><div className="cacts">
          <button className="go" disabled={busy} onClick={() => void perform(async () => { const next = { id: crypto.randomUUID(), at: new Date().toISOString(), quantity: 3 }; setEvent(next); await post('event', next); })}>Deliver signed stock event</button>
          <button disabled={busy || !event} onClick={() => void perform(() => post('event', event))}>Replay same event</button>
        </div>{data.inbox?.length ? <p className="quiet">{data.inbox.length} durable receipt(s). {data.tasks?.length ?? 0} manager task(s).</p> : null}
        {data.tasks?.map((task) => <details key={task.id}><summary>{task.id}: {task.name}</summary><pre>{task.description}</pre></details>)}</section>}
        {data?.rules && <section className="cblock"><h2>Rules and revisions</h2>
          <ul className="crules">{data.rules.active.map((rule) => <li key={`${rule.id}/${JSON.stringify(rule.scope)}`}><span className="id">{rule.id} v{rule.version}</span> / {rule.type} / {rule.enabled ? 'enabled' : 'disabled'}<p>{rule.text}</p>
            {rule.scope.connectionId && rule.enabled && <div className="cacts"><button disabled={busy} onClick={() => void perform(() => post('rules/disable', { id: rule.id }))}>Disable {rule.id}</button></div>}</li>)}</ul>
          <div className="cacts"><button disabled={busy} onClick={() => void perform(async () => setRevision(await post('rules/revise')))}>Propose improvement from corrections</button>
          ...
          {revision && <div aria-label="Rule revision proposal"><h3>{revision.proposal.review?.kind} proposal</h3><p>{revision.proposal.rule?.text}</p><p className="quiet">{revision.proposal.preview.authority}</p>
            <p className="quiet">Offline deterministic replay; no model or network call. This comparison does not predict provider compliance.</p>
            <div className="ctable"><table><thead><tr><th>Case</th><th>Before</th><th>After</th></tr></thead><tbody>{revision.proposal.review?.replay.map((row) => <tr key={row.case}><td>{row.case}</td><td>{row.before}</td><td>{row.after}</td></tr>)}</tbody></table></div>
            <div className="cacts"><button className="go" disabled={busy} onClick={() => void perform(async () => { await post('rules/adopt', { id: revision.proposal.id, digest: revision.digest }); setRevision(null); })}>Adopt reviewed revision</button></div></div>}
        </section>}
        {latestRun && <section className="cblock"><h2>Latest investigation</h2><p>{latestRun.state} / {latestRun.used.modelCalls} scripted model calls / {latestRun.used.toolCalls} tool calls</p><pre>{JSON.stringify(latestRun.result, null, 2)}</pre>
          <details><summary>Inspect raw observations, effective context and rule decisions</summary><pre>{JSON.stringify(latestRun.steps, null, 2)}</pre></details></section>}
      </>}
      <details className="cblock"><summary>Review generated connection examples</summary>
        ...
        <div className="cacts"><button disabled={busy} onClick={() => void perform(async () => setCandidates(await api(`${base}/compiler`)))}>Load generated candidates</button></div>
        {candidates.map((candidate) => <div key={candidate.name}><h3>{candidate.label}</h3><p>Resources: {candidate.resources.map((item) => item.name).join(', ')}. Read-only local fixture.</p>
          ...
          <div className="cacts"><button className="go" disabled={busy || candidate.installed} onClick={() => void perform(async () => {
