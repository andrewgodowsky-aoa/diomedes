...
export interface PreviewCut {
  ...
  /** An inline `visual` block is still being written: shown as its own placeholder, never half-parsed. */
...
export function gatePreview(input: string): PreviewCut {
  ...
  if (last && last.type === 'code' && !last.closed) {
    if (last.lang === 'visual') return { text: lines.slice(0, last.line).join('\n'), pending: null, visualPending: true };
