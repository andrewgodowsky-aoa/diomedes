...
**Technical contract:** Personal and Business use the same strong Core/Runtime/Trust foundations. Personal remains a capable self-managed local harness over the person's own AI tools and direct engines; the Nectovia Agent (Diomedes-led native work) is a paid Business feature, started only for a business whose plan or grant includes it, and a working provider connection never substitutes for that. Business adds organization membership/configuration, maintained Agents/Teams/rules, managed inference, business connectors/governance, support and operational services. Guided/Standard/Technical presentation changes information density, not safety or authority. Technical depth remains available; ordinary users are not required to understand internal vocabulary.
