...
| **`--dm-texture-opacity` drives exactly one layer.** `client/styles.css:276-282` paints `.dm-texture-layer` with it and `client/App.tsx:511-516` mounts that layer inside `.app`. | That slot is the only painted texture in the app; nothing else reads the channel, and `data-texture='off'` zeroes it. A theme author expecting texture anywhere else gets nothing. | A2 report, concern 2; A4 |
...
| **The Website Studio probe believes anything that answers like the studio.** `server/website-studio.ts:96-102` refuses a listener that is not the studio, with its own sentence — but a foreign process on 4400 that replies to `/api/health` with `{ok:true, studio:'website'}` is reported as reachable. | Nothing is sent to it and nothing is read from it beyond that answer, so the cost is a wrong sentence rather than an exposure. There is no shared secret on loopback to do better. | A4 report, concern 5 |
...
| **A theme applies only in the workspace it was applied in.** The applied-theme pointer lives in the one global settings file and carries the scope it was written in; theme storage is per scope. | Switching workspace shows the built-in appearance with no notice, and switching back restores the theme. The pointer is never cleared by the switch. A pointer written before this release carries no scope and is still honoured wherever it is read. Settings → Appearance and Settings → Design Center follow what is painted, not the pointer, so in a workspace where the theme was not applied they show the built-in scheme as the selected one and say nothing about a theme; picking a scheme there leaves the other workspace's theme applied. (2026-09-18) | Final review I2 and its follow-up ruling; `tests/themes.test.ts`, `tests/design-studio-ui.spec.ts` D16 |
| **Every refused write inside `store.locked` costs a store reload.** | Pre-existing `locked` behaviour, not introduced here. The Design Center's own mutating routes now settle the capability and the theme name *before* the lock, so a refusal there costs nothing; the editor also validates before sending. | A2 report, concern 6; final review I1 |
