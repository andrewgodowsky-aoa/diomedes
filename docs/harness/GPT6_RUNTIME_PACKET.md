...
- On `reconcile_required`: Session `waiting`, Task `reason: 'went-wrong'`, the reconcile sentence from `presentRun`, and no automatic restart. The service already refuses `complete` and ignores `fail` for a parked run, and `cancel` parks an in-flight step with an effect instead of relabelling it; the bridge must not write a `done` Session for such a run. A person may cancel or, once you build it, reconcile with an explicit receipt.

### 3. Routes: `server/harness/routes.ts` (new) plus one mount line in `server/app.ts`

- `GET /api/projects/:id/harness/runs` (ids, states, capability, task, session), `GET .../runs/:runId` (the `HarnessRun` passed through the host's secret scrubber, the same `redact` given to the `RunService`; transcripts are references only), `GET .../runs/:runId/events?after=<seq>` (versioned events after the cursor; this is the reconnect path), `POST .../runs/:runId/cancel` (locked, `X-Diomedes-Client` required like every non-GET; calls `runs.cancel(runId, reason, principal)` with the person's principal so the scope check runs).
- Construct the `RunService` with `redact` set to the same scrubber `NativeWorkService` uses for team tokens (`run.redact`, `server/native-work.ts:242`), extended to any secret the host leases into a run.
