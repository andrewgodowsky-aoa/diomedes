...
| Evidence | History page (`HistoryEntry`), Console Changes, the Session log | Harness events are the operational stream (hashes, attempts, fences, never outputs); document evidence stays `HistoryEntry.files`. Step outputs are persisted observations on the run record and error messages are redacted by the host's scrubber before they are written; the run route in the GPT-6 packet must pass the record through the same scrubber. |
...
- **Behaviour:** intent hashing, checkpoint reuse, changed-intent refusal, bounded attempts, stable idempotency key for `idempotent` steps, `reconcile_required` on an unknown non-idempotent outcome, lease claim with fence and TTL (a live owner renewing keeps its fence), stale-owner refusal on commit and on failure, completion event in the same write as the result, versioned events with a cursor. A parked `reconcile_required` run outranks `fail`, refuses `complete`, and `cancel` parks an in-flight step with an effect rather than relabelling it. Error messages pass through the host's `redact` before persistence.
- **Tests:** `tests/harness.test.ts` (the "durable step boundary" block and the child-process crash test: a real process exits with status 17 after writing a receipt; the reopened service refuses to repeat) and `tests/harness-negative.test.ts` (forty refusals, written by Muse from a brief). Totals on the branch: 51 + 8 + 40 harness tests.
...
- **Tests:** `tests/harness.test.ts` "native loop over an injected model adapter" (trajectory reuse, unknown tool, a registered tool outside the capability, schema rejection, turn limit, missing permission, an effect that fails inside the loop, transcript separation, registry description).
- **Depends on:** H01, H05.
