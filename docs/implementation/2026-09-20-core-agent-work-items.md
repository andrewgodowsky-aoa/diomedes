...
### Round 13: admission accepted, composed oracles green, client diagnosis narrowed (2026-09-21)

**The executing CD-01 successor review accepts the bounded repair at exact `7f1b9b3`.** The immutable report is `2026-09-21-core-agent-admission-review-successor.md`, review commit `ef9b51f`. Original archived hashes match, matrix 13/13, unfiltered regression fixtures 99/99, repair plus seven schedules 9/9. Typecheck passes after adding the missing control-plane dependency junction; its initial missing-module failure is retained. Full Vitest is honestly red at that old candidate: 4,266 passed, 1 failed, 1 skipped. The sole failure is the pack-index bug already repaired on main. The soft-deleted-task concern is not a reachable team-board race: both admission, including preparation, and the MCP delete hold the same Store lock. This is a source-trace ruling, not a newly executed delete schedule. Producer mutation results remain producer evidence.

**The preserved composition passed independent execution at exact `2b3f933`.** `2026-09-21-core-agent-composed-review-baseline.md`, review commit `b01b4d8`, records 80/80 across the unchanged AWS review (11), admission matrix (13), seam review (13), capability record (23) and pack-index regression (20). This also independently confirms the main-branch fix for the admission-only candidate's one full-suite failure. It is acceptance of these composed oracles, not a substitute for final full gates on the expanded PR. The outgoing integrator's full-suite/build/browser counts are still labelled as that integrator's run.

**CD05-R-13 is now localized to the browser request-interception transition.** An observational diagnostic copy produced 15 passes and 1 failure: after B's successful POST, the browser emitted B's state GET, but Node never received it before the 12-second assertion deadline. Locks were empty, no claim remained, and the page still showed Working. The otherwise identical variant keeping interception active passed 16/16. This supports a narrow test-harness repair without asserting an upstream defect mechanism or attributing the outgoing integrator's missing historical trace. A different author owns that repair; independent review, repeated closure, full page spec and guard-removal checks remain due. R-5 stays unedited and CD-05b stays unaccepted until that review closes the finding.

**Home Luna and Stop is now in contract drafting, not implementation.** Andrew's `Include in PR #29` controls scope. The isolated lane is `feature/core-agent-home-luna` at `F:/Diomedes/diomedes-wt/core-agent-home-luna`, with a bounded contract author and an independent reviewer. The frozen Claude-only rules must be explicitly superseded; no automatic fallback, silent Work-route change, credentials or live call is included. Canonical cloud amendments remain pending their owner reconciliation.

**Delegation route corrected from the historical table above:** this continuation verified and used SWE 2.0 Max through the installed Devin CLI, model ID `swe-2-max`. It did not use OpenCode for these assignments. The parent holds coordination identity and performs all tests under its own heavy slot. The review reports and this ledger are committed and pushed through the single integration PR #29, which remains draft. No main merge, version bump, package, release or site deployment has occurred.

### Round 14: client closure accepted; Home contract remains under review (2026-09-21)

**CD05-R-13 and the original CD-05b scope are independently accepted.** Candidate `924066b`, review `069cbd5`, and the immutable `2026-09-21-core-agent-client-review-r6.md` preserve the earlier failures. The corrected candidate passed typecheck, build, the repaired closure 32/32 and the complete page spec 33/33. B17 removal failed the pending-count assertion in the repaired closure; B18 removal failed the stale-notice assertion. Each source restoration matched the original SHA-256, was rebuilt, and passed all four R-12 cases. The first author submission's two type errors and a secondary B17 cleanup error remain disclosed. No application code changed. The frozen original R-4 reproducer remains byte-for-byte intact. These checks do not accept the new Home scope or mark the package DONE before main merge.

**Home Luna/Stop has a proposed contract, not accepted code.** SWE 2.0 Max through Devin authored the proposal from frozen source after the first tool-reading assignment timed out without an artifact. The independent review requires a correction to the consume-once, in-memory cancellation proposal and precise Home migration and client ownership behavior before implementation. No silent fallback, Work-route policy change, provider setup or paid call is included.

**Andrew authorized the narrow AWS file transfer.** His exact answer was `The owner has stopped editing; transfer this file`, in response to the handoff for `server/harness/model-session-run.ts`. The coordinator's legacy `fable` role denotes the integrator seat only, not a model delegation. The old indivisible AWS claim was split through the pinned tool: `claim_mub7hn6w_6940be2f` grants only that file to this Home lane; `claim_mub7hmai_bccd8aed` reserves all residual paths for the original AWS owner and grants this task no editing scope over them. The AWS worktree, credentials, live call and spend decision remain outside this task. Both records name this process and the authorization.
