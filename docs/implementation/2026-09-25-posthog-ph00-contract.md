...
A faux (test) account service yields `internal-synthetic` with `synthetic: true`. A cloud service yields `internal`. Rechecked at `enqueue` and immediately before each send by `recheckScope`: same person, same active workspace as when the admission was asked (read before its round trip, PH-07 R3-3), cached entitlement `agent && state === 'active'`, and the organization still internal.

> **Architect ruling 2026-09-25, provisional (PH-07 N-3).** These are not point-in-time comparisons. As 4.6 says, a sign-out, a different person or an active-workspace change since bind ends the scope for good, and its unsent events are dropped. The host sees each such change when it happens (every settings save and every account projection), not only at the next recheck, so undoing it before a flush does not revive the scope. A failed recheck is likewise final for that scope; only an inability to ask (`account-service-unavailable`) is not. An access read that fails in transport, by status or in parsing is an inability to ask, exactly as a read that throws: the session reports it as `unknown`, and only a well-formed answer that the business is not entitled is final (PH-07 R3-1). "Since bind" is measured from when the admission was asked, before its round trip, so a switch while the account service answers ends the scope rather than becoming its baseline (PH-07 R3-3). Coming back to the business, or signing in again as the same person, starts a fresh scope at the next admission and never revives the old one's queue. Not verified: that the machine is actually a company machine (D3). That is bounded by the capture credential living only in operator configuration (PH-02) and by labelling `source_trust`.
...
> **Architect rulings 2026-09-25, provisional (PH-07 N-1, N-2).** "The next admission" means a *definitive* refusal: the account service's code is `entitlement_revoked`, `entitlement_expired`, `agent_not_included` or `not_a_member`. The gate's error carries that code as an additive, read-only, non-enumerable `refusalCode`; its class, `code`, message and HTTP status are unchanged. An outage (`entitlement_unknown`: network error, timeout, 5xx, or a 403 or 404 that is not the service's own membership refusal) or a sign-in refusal ends nothing. `not_a_member` is only the service's own refusal: a decision with that code, or its 403 membership answer (PH-07 R3-2). The business a refusal ends is the one the gate asked about, decided once before the round trip to the account service by the gate's own rule (the project's owner, else the active business), and never re-derived after it.
...
- **The scope map's bound (2,000) never forgets a scope a live run has claimed** (PH-07 R3-4). Past it, the oldest scope whose run ended goes first, then the oldest no run has claimed (counted `scope-evicted`). A run saved after its scope was forgotten is counted in the projector's `scopeEvicted`, so the loss is never mistaken for a lost trace.
