...
| 4 | Direct Ask / Plan (`POST /api/projects/:id/ask`) | Claude Code, OpenCode, Oh My Pi, Cursor, Devin (one-shot `generate`); Codex (`askCodex`) | yes | yes, scoped to selected documents | person-selected playbook (`assembleSkillSection`), unchanged | external engines: the text route run's intent records `instructions` verbatim (`engines/service.ts:1566`); Codex: none (the Ask route keeps no run for Codex). Codex sends `instructions` as the thread's `baseInstructions` (`integrations.ts:1308-1313`) | **none** | "a direct Ask is sent the rules in its instruction channel"; `answer-format.test.ts` updated to the new shape |
...
- `messageRules` reads the applied instruction files while the conversation host holds the store
  lock, as H11's Work path reads them during admission, and loads the shipped product knowledge
  bundle (two small JSON files) each time, as every `assembleInstructions` call already does.
  Only instruction files the rule path applied are read.
