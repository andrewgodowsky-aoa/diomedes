...
| Case id | Case (short) | Verdict | Evidence |
|---|---|---|---|
| J65 | Ordinary Free uses a company-funded endpoint | COVERED | `tests/managed-gateway.test.ts:118` — every managed admission is refused as `no_entitlement` in this build, while line 164 admits a business's own key and debits nothing, so the separately authorized option survives the denial |
...
| J73 | User attempts to set local paid-plan flags | COVERED | `tests/managed-gateway.test.ts:206` — a request body asserting paid access, a managed entitlement and another company's id is refused as `no_entitlement`, because the gateway takes the organization from the host's own resolution |
