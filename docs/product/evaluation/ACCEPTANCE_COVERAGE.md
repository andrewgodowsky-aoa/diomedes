...
| J50 | A hundred equivalent error events arrive | GAP | `tests/evaluation-step.test.ts` — must show equivalent events coalesce by causal step identity into one recommendation under a bounded queue. The nearest existing assertion is `tests/reviewer-authority.test.ts:350`, where one change set buys one reviewer call however often admission is retried, but it coalesces retries rather than an error storm |
...
| J53 | Required verification times out | COVERED | `tests/reviewer-authority.test.ts:322` — a reviewer that never answers is recorded as an error with reason `cancelled`, the authorization is absent, the proposal is preserved and the file is never written |
