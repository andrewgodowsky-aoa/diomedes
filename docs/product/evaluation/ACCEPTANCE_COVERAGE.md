...
| J53 | Required verification times out | COVERED | `tests/reviewer-authority.test.ts:336` — a reviewer that never answers is recorded as an error with reason `cancelled`, the authorization is absent, the proposal is preserved and the file is never written |
