...
> **Retired prices removed, 2026-09-19.** This is a dated archive of a whole document. By owner direction, Diomedes' own retired price figures were struck from it and appear as "[retired price removed]"; nothing else was changed. Reason and scope: `docs/business/PRICING_STRATEGY_2026-09-15.md`, "Removed figures". Current prices live only in that document. Git history holds the original.
...
Current entry offer, pricing version 2026-09-15.2: the 90-Day Workflow Starter is [retired price removed]/month for 3 months ([retired price removed] total), with no separate Business subscription during the program. Scope is one supported workflow, one location/unit, up to two supported sources/connections and 60-90 minutes remote onboarding, reasonable rules, bounded usage, human review and handoff. Custom connectors/integrations, substantial cleanup, on-site implementation, major engineering, unlimited support, priority incident response and an SLA are excluded. After 90 days there is no automatic continuation: stop, or separately agree planned self-managed Business from [retired price removed]/org/month once operational, or Managed from $750/month with scope and availability confirmed. Billing and operational managed service remain in development. This is a distinct bounded entry engagement; it does not lower the professional implementation anchors below.
...
The September 15, 2026 commercial decision supersedes the older price anchors. Diomedes Business is planned from [retired price removed]/month per organization, but the exact included managed-inference allowance remains unresolved and must be cost-qualified rather than inherited from the older illustrative $300/$100 candidate. Entitlement, Trust permission and budget admission are different checks. Managed inference uses company-held server credentials and tenant-bound reserve/settle accounting including Team/reviewer/correction calls. Local/BYO is not silently charged against that allowance. No unlimited usage, automatic overage, invisible payer fallback or assumption that online inference provides an always-on execution host.
...
Files and Artifacts (2026-09-10.7) are Core, not a software feature. Every Project has one Files surface carrying the project folder and artifact model, ordinary file preview, text/Markdown viewing, search, generated artifacts, history/version inspection, attachments and references into Threads, and preview or open-externally behaviour for common business files: SOPs, reports, exports, documents, spreadsheets, invoices and images are ordinary business objects. The IDE-grade version of that same surface is supplied by a capability pack on activation and is never a second application surface.

A capability pack (2026-09-10.7) is how Diomedes becomes good at a kind of work without becoming a product for that kind of work. A pack may affect the combination of tools, Agents, rules, context, workflows and relevant UI affordances, not merely prompt text, while using the same Core Runtime, Trust and Project contracts. It never introduces a parallel runtime, permission model, file authority or application surface. Activating a pack is not an authorization event: the pack declares the capabilities it would use and Trust still decides, exactly as changing Agent, model or Team never grants authority. Packs activate per Project; a workspace may offer, recommend or require one. A pack's toolset must not be injected or loaded for users or projects that do not need it, and a pack-supplied Agent's work keeps truthful runtime attribution and is recorded as pack-supplied rather than presented as Core behaviour. The Software Engineering pack is the first, supplying repository awareness, code reading and navigation, diffs and changed-file review, diagnostics, test/build commands, worktrees and branches, repo-aware context construction and software-specific Agents. Repository instruction files such as AGENTS.md and CLAUDE.md are discovered and fed through the standing-guidance context and rule path rather than pasted into a prompt, with their scope, source and version recorded and an inspectable `Project instructions loaded · AGENTS.md` indication; a rule the user cannot inspect is a hidden behaviour change, not standing guidance.
