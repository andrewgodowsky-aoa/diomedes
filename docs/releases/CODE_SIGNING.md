...
Written 2026-09-12; routes and prices rechecked 2026-09-25. Status: **nothing is signed yet.** Every published Diomedes build is
...
| **OV or EV certificate from a CA** (Sectigo, DigiCert, SSL.com, Certum) | A traditional code-signing certificate issued to a validated individual or organization. Since June 2023 the key must live on a hardware token or HSM; certificates issued after March 2026 are valid at most 460 days. | OV from about US$115 a year for Certum Code Signing in the Cloud (key held in Certum's cloud HSM, no token; signing goes through the SimplySign app) and about US$219 a year for Sectigo through resellers; EV US$226 (Certum cloud) to US$685 a year. Token shipping extra unless a cloud HSM is used. | Buy, pass validation (OV: identity documents; EV: registered business), receive a USB token, install its driver, sign on that machine. | Same as above after the 2024 SmartScreen change; EV no longer skips the warning. EV is required only for kernel drivers. | Workable. Certum's cloud OV costs about the same per year as Artifact Signing Basic and needs no token. Choose it if Azure identity validation is refused, or if Azure will only validate you as an individual and the publisher line must read Diomedes Systems LLC. |
| **SignPath Foundation** | Free signing for open-source projects; the certificate belongs to SignPath Foundation, so *they* are the publisher shown to users. | Free. | **No longer available to Nectovia.** Its terms require an OSI licence for every component; the repository moved from Apache-2.0 to an interim all-rights-reserved notice on 2026-09-25 (PR #135). The original terms, for the record: OSI licence for every component, binaries built from source "in a verifiable way" through their pipeline, each release manually approved, a code-signing policy published on the download page naming SignPath and the team roles, MFA on the repository and on SignPath. | Reputation accrues to SignPath Foundation's certificate. | Ruled out since 2026-09-25. It also made the publisher line read "SignPath Foundation", it requires the build to move into their pipeline, and the bundled third-party engine binaries would need review against their no-proprietary-components rule. Not tonight's path. |

Organization validation for a young company: in August 2026 a Microsoft employee answered on Microsoft Q&A that Artifact Signing has "no minimum org age restrictions", while older guidance asked for three years of verifiable history. Diomedes Systems LLC is new, so apply as an organization first and fall back to individual validation (publisher shown as Andrew's legal name) or a Certum organization certificate if it is refused.
...
ssldragon, ssl.com). Rechecked 2026-09-25: SSLmentor's Certum cloud listings, and the Microsoft Q&A answer on organization age (learn.microsoft.com/en-us/answers/questions/5977141). Prices change; recheck before paying.
