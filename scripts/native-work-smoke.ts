...
try {
  ...
  assert.ok(need.approval, 'The native proposal must carry an exact approval identity.');
  const approvalCommand = {
    protocolVersion: 1 as const,
    commandId: 'native-work-smoke-approval',
    resolution: 'go-ahead' as const,
    proposalDigest: need.approval.proposalDigest,
    actionDigest: need.approval.actionDigest,
    baseDigest: need.approval.baseDigest,
  };
  const decided = await api<import('../shared/types.js').Need>(
    `/projects/${project.id}/needs/${need.id}/resolve`,
    'POST',
    approvalCommand,
  );
  const replayed = await api<import('../shared/types.js').Need>(
    `/projects/${project.id}/needs/${need.id}/resolve`,
    'POST',
    approvalCommand,
  );
  assert.ok(decided.approvalReceipt, 'Approval must return its durable decision receipt.');
  assert.deepEqual(replayed.approvalReceipt, decided.approvalReceipt);
  assert.equal(decided.execution?.state, 'applied');
  proof.approvalReceipt = decided.approvalReceipt;
  proof.approvalExecution = decided.execution;
  assert.equal(
    await fs.readFile(path.join(projectFolder, 'Brief.md'), 'utf8'),
    expected,
