...
const allowed = new Set(['dist', 'main.mjs', 'app-updates.mjs', 'update-helper.mjs', 'fresh-start.mjs', 'server', 'fixtures', 'licenses', 'package.json', 'LICENSE', 'BUILD_INFO.json']);
...
const manifest = { schemaVersion: 1, product: 'Diomedes', releaseId, appVersion: '0.1.1', channel: 'experimental',
  ...
  internalPackageHashes: { executableSha256: await sha(path.join(packageRoot, 'nectovia.exe')), asarSha256: await sha(asar), files: files.length, payloadTreeSha256: installer.appTreeSha256 },
