...
export class ConnectionsService {
      ...
      for (const operation of adapter.operations) {
        ...
        this.host.tools.register({
          ...
          outputSchema: z.strictObject({
            result: z.json(),
            corrections: z.array(z.json()),
            connectorVersion: z.string(),
            assurance: z.literal('fixture-only'),
        ...
        async ({ signal }) =>
          this.mutate(projectId, async (state) => {
            ...
            receipt.rules = evaluateRules(
              ...
              { ...observation.facts, sourceAt: observation.sourceAt },
            );
  ...
  /** Explicit host-session renewal. Accepted observations remain data, never a
   * restored grant. Changed identity gets a NEW run; the old run and output stay
   * intact, and the Store receipt remains the authority for an already-made Task. */
  async resumeAuthorized(projectId: string, connectionId: string) {
    const connection = this.find(this.data(projectId), connectionId);
    const current = await this.authority(connection, 'connections.manage');
    if (!current.capabilities.includes('issues.create')) deny('authority_denied', 'Current scope cannot create a manager task.');
    const saved = await this.host.list(projectId);
    for (const receipt of this.data(projectId).connections.inbox.filter(item => item.connectionId === connectionId && item.state !== 'ignored')) {
      const binding = this.data(projectId).connections.bindings[receipt.runId];
      const known = saved.find(run => run.id === receipt.runId);
      if (!binding || (binding.principalId === current.id && binding.identityGeneration === current.identityGeneration)) continue;
      if (known && !['completed', 'cancelled'].includes(known.state)) {
        // This host-only operation withdraws execution; it grants no old rights.
        await this.host.runs.cancel(known.id, 'Superseded by explicit current-session connection authorization; durable effect receipt retained.');
      }
      if (receipt.state === 'pending') await this.mutate(projectId, async (state) => {
        const fresh = this.find(state, connectionId), principal = await this.authority(fresh, 'connections.manage');
        if (principal.id !== current.id || principal.identityGeneration !== current.identityGeneration || fresh.generation !== connection.generation)
          deny('connection_revoked', 'Authority changed during event recovery.');
        const event = state.connections.inbox.find(item => item.connectionId === connectionId && item.id === receipt.id)!;
        if (event.state !== 'pending' || event.runId !== receipt.runId) return;
        event.priorRunIds = [...(event.priorRunIds ?? []), event.runId];
        event.runId = `ce-${digest([event.id, connectionId, current.id, current.identityGeneration, fresh.generation]).slice(0, 32)}`;
      });
    }
    await this.mutate(projectId, async (state) => {
      const fresh = this.find(state, connectionId); await this.authority(fresh, 'connections.manage');
      fresh.lastReconciledAt = null; fresh.reconciledResources = {};
    });
    await this.recover(projectId);
    return copy(this.find(this.data(projectId), connectionId));
  }
  ...
  async activate(
    ...
    return this.mutate(projectId, async (state) => {
      ...
      if (
        ...
        digest(proposal.rule.scope) !== digest(this.scope(connection))
      )
      ...
      if (proposal.review && (this.clock() >= Date.parse(proposal.review.expiresAt) ||
        !previous || digest(previous) !== proposal.review.baseDigest))
        deny('revision_stale', 'The reviewed rule revision expired or its base changed.');
      const rule: Rule = {
        ...proposal.rule,
        enabled: true,
  ...
  async modelContext(request: ModelRequest) {
    ...
    return this.mutate(connection.projectId, async (state) => {
      ...
      this.credentials.assertSafe(text);
      if (text.length > 8000) deny('context_budget', 'Scoped guidance exceeds the 8000-character context budget.');
      return { text, tools };
    });
  }
  async runFixtureAgent(runId: string, adapter: ModelAdapter, prompt: string) {
    ...
    const scoped = new RuleModelAdapter(
      ...
      async (request, text) => {
        const { connection, binding, principal: current } = await this.bound(request.runId, 'connections.read');
        const state = this.data(connection.projectId);
        const unknown = Object.entries(state.connections.observations).filter(([key, observation]) =>
          key === observationKey(connection.id, observation) && binding.resources.includes(observation.resourceId) &&
          observation.facts.quantityUnknown === true).map(([, observation]) => observation);
        // Deliberately bounded final-text inspector; no streaming/tool-call interception claim.
        const invalid = unknown.length > 0 && /(?:untracked|not tracked|unknown)[^.]*\b\d+(?:\.\d+)?\s*(?:units?|remaining|items?)/i.test(text);
        const corrections = unknown.flatMap((observation) => evaluateRules(state.rules.active,
          this.scope(connection, { resourceId: observation.resourceId, role: binding.role,
            userId: current.id, capabilityId: binding.operations[0], runId: request.runId }),
          observation.facts)).filter((rule) => rule.action === 'correct');
        return { action: invalid ? (corrections.length ? 'correct' as const : 'refuse' as const) : 'verified' as const,
          message: invalid ? 'Correction required: the source did not report a numeric quantity for the untracked item. State that its quantity is not tracked. Do not infer a number.'
            : 'Bounded unknown-quantity check passed; raw source observations are preserved.',
          rules: corrections.map(({ id, version }) => ({ id, version })) };
      },
    );
  ...
  async finishRead(runId: string, result: Json) {
    await this.bound(runId, 'connections.read');
    await this.host.runs.complete(runId, this.owner, result);
  }

  /** Host-reviewed local rule proposal. HTTP callers never choose scope or authority. */
  async proposeMonitor(projectId: string, connectionId: string, threshold: number,
    window: z.infer<typeof serviceWindowSchema>) {
    const proposal = await this.propose(projectId, connectionId,
      `Alert a manager when reported quantity is at most ${z.number().finite().min(0).max(100000).parse(threshold)}`);
    return this.mutate(projectId, async (state) => {
      const connection = this.find(state, connectionId);
      await this.authority(connection, 'connections.manage');
      const rule = ruleSchema.parse({ ...proposal.proposal.rule, serviceWindow: serviceWindowSchema.parse(window) });
      const revised = { ...proposal.proposal, rule,
        preview: { ...proposal.proposal.preview, when: `${proposal.proposal.preview.when}; ${window.start}-${window.end} ${window.timeZone}, days ${window.days.join(',')}` } };
      revised.id = digest(revised).slice(0, 24);
      if (!state.rules.proposals.some((item) => item.id === revised.id)) state.rules.proposals.push(revised);
      return { proposal: revised, digest: digest(revised) };
    });
  }

  async proposeGuidance(projectId: string, connectionId: string) {
    return this.mutate(projectId, async (state) => {
      const connection = this.find(state, connectionId);
      await this.authority(connection, 'connections.manage');
      const rule = ruleSchema.parse({ id: 'stock-investigator-notes', version: 1, enabled: false,
        scope: this.scope(connection), type: 'standing', action: 'context', predicate: null,
        provenance: { source: 'user proposal', connectorVersion: null, trust: 'candidate' },
        text: 'Use concise manager reports. Distinguish menu availability from physical ingredient inventory.' });
      const proposal = { id: digest(rule).slice(0, 24), state: 'proposed' as const,
        sourceText: 'Use concise manager reports.', rule, questions: [],
        preview: { when: 'Before a scoped model call', do: rule.text, appliesTo: connection.name,
          authority: 'Instruction only; no additional tools or rights', enforcement: 'Frozen model context' } };
      if (!state.rules.proposals.some((item) => item.id === proposal.id)) state.rules.proposals.push(proposal);
      return { proposal, digest: digest(proposal) };
    });
  }

  async proposeRevision(projectId: string, connectionId: string, rollbackVersion?: number) {
    // Offline evaluation consumes only durable local evidence. It has no model or network port.
    const runs = await this.host.list(projectId);
    const sourceSteps = runs.filter((run) => this.data(projectId).connections.bindings[run.id]?.connectionId === connectionId)
      .flatMap((run) => run.steps.filter((step) => step.intent.name === 'inspect_model_output' &&
      step.output && JSON.stringify(step.output).includes('"action":"correct"'))
      .map((step) => `${run.id}/${step.intent.stepId}`)).slice(-50);
    return this.mutate(projectId, async (state) => {
      const connection = this.find(state, connectionId);
      await this.authority(connection, 'connections.manage');
      const before = state.rules.active.find((rule) => rule.id === 'stock-investigator-notes' && rule.scope.connectionId === connectionId);
      if (!before || (!sourceSteps.length && rollbackVersion === undefined))
        deny('correction_evidence_required', 'Run the bounded correction demonstration before proposing an improvement.');
      const older = rollbackVersion === undefined ? null : state.rules.revisions.find((rule) =>
        rule.id === before.id && digest(rule.scope) === digest(before.scope) && rule.version === rollbackVersion);
      if (rollbackVersion !== undefined && !older) deny('revision_missing', 'That historical version is unavailable.');
      const rule: Rule = { ...copy(before), enabled: false,
        text: older?.text ?? `${before.text}\nFor an untracked item, say "quantity not tracked". Never infer a numeric amount.`,
        provenance: { ...before.provenance, trust: 'candidate' } };
      ruleSchema.parse(rule);
      // This revision API permits prose only. Scope, predicate, effect and capabilities cannot expand.
      const scoped = this.scope(connection);
      const replacement = state.rules.active.map((item) => item === before ? { ...rule, enabled: true,
        provenance: { ...rule.provenance, trust: 'host-reviewed' as const } } : item);
      const cases: { name: string; facts: Record<string, Json> }[] = [
        { name: 'Untracked quantity', facts: { quantity: null, quantityUnknown: true, externalWrite: false } },
        { name: 'Reported quantity 3 at replay time', facts: { quantity: 3, quantityUnknown: false,
          externalWrite: false, sourceAt: this.now() } },
        { name: 'Proposed external write', facts: { externalWrite: true } },
      ];
      const outcome = (rules: Rule[], facts: Record<string, Json>) => {
        const matches = evaluateRules(rules, scoped, facts);
        const instructionCoverage = selectRules(rules, scoped).some((item) => item.type === 'standing' &&
          item.text.includes('quantity not tracked'));
        return JSON.stringify({ matches: matches.map(({ id, action }) => ({ id, action })),
          explicitUntrackedGuidance: instructionCoverage,
          authority: 'unchanged; replay cannot dispatch effects' });
      };
      const review = { baseDigest: digest(before), sourceSteps,
        replay: cases.map((item) => ({ case: `${item.name}: ${JSON.stringify(item.facts)}`,
          before: outcome(state.rules.active, item.facts), after: outcome(replacement, item.facts) })),
        expiresAt: new Date(this.clock() + 15 * 60000).toISOString(),
        kind: rollbackVersion === undefined ? 'improvement' as const : 'rollback' as const };
      const proposal = { id: digest({ rule, review }).slice(0, 24), state: 'proposed' as const,
        sourceText: `${review.kind} from durable correction history`, rule, questions: [], review,
        preview: { when: 'Next scoped reasoning step after adoption', do: rule.text, appliesTo: connection.name,
          authority: 'No change to tools, scope, permissions, destinations or credentials', enforcement: 'Reviewed instruction; mandatory policy and output verification remain' } };
      state.rules.proposals.push(proposal);
      return { proposal, digest: digest(proposal) };
    });
  }
