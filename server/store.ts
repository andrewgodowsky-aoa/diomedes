...
export class Store extends EventEmitter {
  ...
  workCommand(id: string, commandId: string, payloadDigest?: string) {
    const session = this.state(id).sessions.find((item) => item.receipt?.commandId === commandId);
    if (session && payloadDigest && session.receipt?.payloadDigest !== payloadDigest)
      throw new ApiError(409, 'This Work command already names a different request.', {
