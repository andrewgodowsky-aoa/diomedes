...
const commandSchema = z.strictObject({
  ...
  review: z.enum(['human', 'model-reviewer']),
  // Reviewer routing is confirmed by the person alongside the scope; a model
  // never selects its own reviewer, and this never widens the scope above.
  reviewer: z
    .strictObject({
      agentId: z.string().min(3).max(80).optional(),
      requestedModel: z.string().min(1).max(120).nullable(),
      maxReviews: z.number().int().min(1).max(200),
    })
    .optional(),
});
...
/**
 * The deterministic result a reviewer decision is recorded against. It states
 * what the host already allowed, so a reviewer is not left guessing which
 * boundary it is standing in front of.
 */
export function describeScopePolicy(
  record: ScopeGrantRecord,
  writes: readonly WriteInput[],
): string {
  const creates = writes.filter((write) => write.expected === null).length;
  return [
    `Deterministic scope policy allowed this change set: ${writes.length} supported text `,
    `${writes.length === 1 ? 'file' : 'files'} (${creates} created, ${writes.length - creates} updated) `,
    `inside ${record.grant.roots.join(', ')} for task ${record.grant.taskId}, via `,
    `${record.grant.engine} on ${record.grant.accountRoute}, within its file and byte budget and `,
    `before ${record.grant.expiresAt}. Deletion, commands, installs and external destinations are `,
    'refused by the host and were not part of this decision.',
  ].join('');
}
...
const grantSchema = z.strictObject({
  ...
  reviewer: z
    .strictObject({
      ...
      agentRole: z.string().min(4).max(2000),
      requestedModel: z.string().min(1).max(120).nullable(),
      maxReviews: z.number().int().min(1).max(200),
      inferenceConsent: z.literal('reviewer-only'),
    })
    .optional(),
...
const scopedAuthorizationSchema = z.strictObject({
  ...
  eventId: z.string().min(1).max(100),
  // Absent means deterministic scope alone authorized this. It never means a
  // person inspected the output.
...
export class ScopeGrants {
  ...
  /**
   * Set once by the host when a reviewer route exists. It is deliberately not
   * a setting, a request field or a preference: nothing a model or a renderer
   * can reach may make `Approve for me` issuable.
   */
  reviewerConfigured = false;
  /**
   * Resolves the reviewer's Agent identity at consent time. Assigned by the
   * host, never taken from a request: a renderer or a model must not be able to
   * nominate the reviewer identity, and only an Agent whose own ceiling is
   * `review` may hold the role.
   */
  reviewerAgent?: (
    agentId: string,
  ...
  async issue(projectId: string, input: unknown): Promise<ScopeGrantRecord> {
    ...
    if ((command.review === 'model-reviewer') !== !!command.reviewer)
      throw new ApiError(
        400,
        'Approve for me needs a reviewer route, and Work in this project takes none.',
      );
    if (command.review === 'model-reviewer' && !this.reviewerConfigured)
      throw new ApiError(
        409,
        'No separate reviewer is connected on this installation, so Approve for me cannot be confirmed.',
    ...
    let reviewer: TaskScopeGrant['reviewer'] | undefined;
    if (command.reviewer) {
      const wanted = command.reviewer.agentId ?? 'diomedes.reviewer';
      const identity = await this.reviewerAgent?.(wanted, state.project.folder);
      if (!identity)
        throw new ApiError(400, `No Agent named ${wanted} is available to review here.`, {
          code: 'reviewer_agent_unavailable',
        });
      // A reviewer identity that could itself write is not a reviewer.
      if (identity.ceiling !== 'review')
        throw new ApiError(
          400,
          `${identity.name} can work under a scope, so it cannot also be the reviewer for one.`,
          { code: 'reviewer_agent_unavailable' },
        );
      reviewer = {
        engine: 'codex',
        ...
        agentDigest: identity.digest,
        agentRole: identity.role,
        requestedModel: command.reviewer.requestedModel,
        maxReviews: command.reviewer.maxReviews,
    ...
    const event = this.store.addEntry(state, {
      ...
      taskId: task.id,
      sentence: reviewer
        ? `You allowed supported text creates and updates for this task (${roots.join(', ')}), with ${reviewer.agentName} reviewing each change set first. The reviewer cannot widen this scope. Sending still requires separate consent.`
        : `You allowed supported text creates and updates for this task (${roots.join(', ')}). Sending still requires separate consent.`,
    });
  ...
  /**
   * The reviewer gate. `mint` asks whether a reviewer approval may create an
   * authorization now: it must be an approve, bound to this exact change set
   * and the live grant generation, and still inside its own short lifetime.
   * `effect` asks whether an authorization that already names a review may
   * still reach the disk: the named review must resolve to that same approve,
   * bound the same way. Lifetime is deliberately not rechecked at `effect` -
   * it bounds when authority may be minted, and a durable authorization is
   * governed by the grant, exactly as a deterministic one is. Revocation still
   * stops it, because a revoked grant no longer matches its generation.
   */
  private reviewGate(record: ScopeGrantRecord, need: Need, mode: 'mint' | 'effect') {
    if (record.grant.review === 'human') {
      if (need.authorization?.reviewId)
        throw failure('This scope routes review to you; it cannot carry a model reviewer decision.');
      return;
    }
    if (!record.grant.reviewer)
      throw failure('This scope names a model reviewer but records no reviewer route.');
    if (mode === 'mint') {
      if (!usableApproval(need, record, scopeGrantDigest(record)))
        throw failure('A separate reviewer has not approved this exact change set.');
      return;
    }
    const named = need.authorization?.reviewId;
    if (!named)
      throw failure('This scope requires a reviewer decision, and this authorization names none.');
    const review = (need.reviews ?? []).find((item) => item.id === named);
    if (
      !review ||
      review.decision !== 'approve' ||
      review.reasonCode !== 'matches-request' ||
      review.decisionSource !== 'model-reviewer' ||
      review.grantId !== record.grant.id ||
      review.grantGeneration !== record.generation ||
      review.grantDigest !== scopeGrantDigest(record) ||
      ...
      throw failure('The reviewer decision for this change set is missing or no longer matches.');
  }
  private async check(
    record: ScopeGrantRecord,
    need: Need,
    ...
    if (gate) this.reviewGate(record, need, gate);
    const latest = this.inactiveReason(record);
    if (latest) throw failure(latest);
  }
  /** Deterministic budget headroom for this grant, excluding this proposal. */
  remaining(projectId: string, grantId: string, needId?: string) {
    const grant = this.store
      .state(projectId)
      .scopeGrants?.find((item) => item.grant.id === grantId)?.grant;
    if (!grant) return { writes: 0, bytes: 0 };
    ...
    return {
      ...
      bytes: Math.max(0, grant.budget.maxBytes - used.reduce((n, item) => n + item.bytes, 0)),
    };
  }
  async matching(
    projectId: string,
  ...
  async record(
    ...
    // The reviewer gate is checked against the live grant before authority is
    // minted, so a revoked or re-generated grant cannot be authorized by a
    // reviewer answer that arrived late.
    if (record.grant.review === 'model-reviewer') {
      const approval = usableApproval(need, record, scopeGrantDigest(record));
      if (!approval || (reviewId !== undefined && approval.id !== reviewId))
        throw failure('A separate reviewer has not approved this exact change set.');
      reviewId = approval.id;
    } else if (reviewId !== undefined)
      throw failure('This scope routes review to you; it cannot carry a model reviewer decision.');
    await this.check(record, need, writes, engine, accountRoute, 'mint');
    const state = this.store.state(projectId);
    const event = this.store.addEntry(state, {
      ...
      sentence:
        record.grant.review === 'model-reviewer'
          ? `This ${writes.length}-file proposal matched the scope you allowed for this task and a separate model reviewer approved it. You did not review this output.`
          : `This ${writes.length}-file proposal matched the scope you allowed for this task.`,
    });
  ...
  async assertCurrent(projectId: string, need: Need, writes: readonly WriteInput[]) {
    ...
    await this.check(record, need, writes, evidence.engine, evidence.accountRoute, 'effect');
  }
}
...
export function validateScopeGrants(state: ProjectState) {
  ...
  for (const record of grants) {
    ...
    // Routing and reviewer route are one fact recorded twice; a mismatch is
    // corruption, and a saved reviewer route never becomes a wider scope.
    if ((grant.review === 'model-reviewer') !== !!grant.reviewer) throw incompatible();
    if (grant.reviewer && grant.reviewer.engine !== grant.engine) throw incompatible();
...
export function validateScopedAuthorization(state: ProjectState, need: Need) {
  ...
  if (
    ...
    fail();
  // Reviewer routing is a fact of the grant, so the authorization must agree
  // with it: a reviewer-routed grant authorizes nothing without naming its
  // review, and a person-routed grant can never name one.
  if ((record!.grant.review === 'model-reviewer') !== !!evidence!.reviewId) fail();
