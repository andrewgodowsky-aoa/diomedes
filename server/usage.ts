...
const DETAILS: Record<string, string> = {
  ...
  codex: 'Codex has not reported its allowance yet. It appears after the next connection check or turn.',
...
function labelFor(id: string, durationMins: number | null): string {
  if (durationMins !== null) {
    ...
    if (durationMins < 10080 && durationMins % 1440 === 0)
      return `${durationMins / 1440} days`;
...
export function windowsFromRateLimits(result: unknown): {
  ...
  const plan = typeof body.planType === 'string' && body.planType.trim() ? body.planType.trim().slice(0, 80) : null;
