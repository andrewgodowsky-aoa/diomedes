...
describe.skipIf(!enabled)('REAL PostgreSQL (explicit disposable database only)', () => {
  beforeAll(async () => {
    ...
    migrations = await Promise.all(['001_accounts.sql', '002_commercial.sql', '003_funded_jobs.sql', '004_usage_contract.sql', '005_customer_access.sql', '007_relay_devices.sql'].map(async (name, index) => {
