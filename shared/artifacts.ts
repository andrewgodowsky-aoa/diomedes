...
/**
 * One file's artifacts. A `.html` file is one design, a `.svg` file is one
 * image and a `.mmd` file is one diagram — each read the way its fence would
 * be, declaration and all — and a `.json` file that holds a valid visual spec
 * is one visual (what Save to Files writes for a visual); any other text is
 * read like a turn.
 */
export function indexFile(path: string, text: string): ArtifactIndex {
  ...
  if (/\.svg$/i.test(path)) {
    const declared = declarationOf('image', text);
    return wholeFile(
      scope,
      {
        identity: declared.id ? `id:${declared.id}` : key,
        ...
        kind: 'image',
        lang: 'svg',
        title: declared.title ?? name.replace(/\.svg$/i, ''),
        source: text,
      },
      key,
    );
  ...
  if (/\.mmd$/i.test(path)) {
    const declared = declarationOf('diagram', text);
    return wholeFile(
      scope,
      {
        identity: declared.id ? `id:${declared.id}` : key,
        ...
        kind: 'diagram',
        lang: 'mermaid',
        title: declared.title ?? name.replace(/\.mmd$/i, ''),
        source: text,
      },
      key,
    );
...
export function fileHasArtifacts(path: string, text: string): boolean {
  if (!/\.(md|markdown|html?|json|svg|mmd)$/i.test(path)) return false;
