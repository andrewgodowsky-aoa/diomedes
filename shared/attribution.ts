...
export interface OriginSnapshot {
  ...
  /**
   * Which Diomedes Agent produced this, independently of the model that ran it.
   * The same Agent keeps its identity across models, and generated text can
   * never set this: the host copies it from the resolved execution snapshot.
   */
  readonly agent?: {
    readonly id: string;
    ...
    readonly selection: 'manual' | 'automatic';
  };
...
export function formatOrigin(
  ...
  return {
    ...
    agentDetail: agentName
      ? `${agentName}${snapshot?.agent?.selection === 'automatic' ? ', chosen automatically' : ''}, version ${displayName(snapshot?.agent?.version)}`
      : '',
    label: `${agentName ? `${agentName} · ` : ''}${primary}${secondary ? ` ${secondary}` : ''}`,
  };
}
