...
test('Console discovers, selects, streams, cancels, and approves all three fixture engines', async ({
  ...
  await page.getByRole('button', { name: 'Settings', exact: true }).click();
  await expect(page.locator('.console')).toBeVisible();
  const picker = page.locator('.picker > button');
  const documentScope = [
  ...
  expect(
    (await api<ProjectState>(`/projects/${project.id}/state`)).conversations[0].turns.some(
      (turn) => turn.role === 'diomedes' && turn.text.includes('CANCEL'),
    ),
  ).toBe(false);
  ...
  for (const engine of Object.keys(versions) as ExternalEngine[]) {
    ...
    const proposal = page.getByRole('dialog', { name: /^Diomedes wants to/ });
  ...
  await expect(page.locator('.history-entry').filter({ hasText: 'Diomedes changed' })).toHaveCount(
    3,
  );
