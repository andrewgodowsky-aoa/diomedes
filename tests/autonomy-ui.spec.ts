...
function proposalText(): { text: string; model: string } {
  ...
  return {
    ...
    text: JSON.stringify({
      ...
      changes: [{ path: 'Autonomy-proof.md', text: '# Autonomy proof\n\nScope fixture first proposal.\n', summary: 'Create the scope fixture proof file.' }],
    }),
  };
}
...
async function api<T>(route: string, method = 'GET', data?: unknown): Promise<T> {
  ...
  if (!response.ok) throw new Error(`Autonomy fixture ${route} failed (${response.status}): ${await response.text()}`);
...
async function makeTaskProject(taskName: string): Promise<{ project: Project; taskId: string; threadId: string }> {
  const project = await api<Project>('/projects/sample', 'POST', {});
  const { found } = await api<{ found: TaskCandidate[] }>(`/projects/${project.id}/plans/find-tasks`, 'POST', {
    path: 'Reopening plan.md',
  });
  const created = await api<{ tasks: { id: string }[] }>(`/projects/${project.id}/plans/add-tasks`, 'POST', {
    path: 'Reopening plan.md',
...
async function openThread(page: Page, taskName: string): Promise<void> {
  await rail(page).getByRole('button', { name: new RegExp(taskName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')) }).first().click();
  await expect(page.locator('#scrThread')).toBeVisible();
}

test('scope confirmation covers the same task; recorded actor survives a picker mutation', async ({ page }, testInfo) => {
  ...
  await expect.poll(async () => fs.readFile(path.join(project.folder, 'Reopening plan.md'), 'utf8')).toBe(beforePlan);
  await expect(async () => {
  ...
  await api('/settings', 'PUT', { services: { codex: true, defaultEngine: 'codex', codexModel: 'different-request' } });
  await page.reload();
  await openThread(page, taskName);
  await expect(page.getByRole('region', { name: 'Needs your OK' }).getByText(SYNTHETIC_MODEL).first()).toBeVisible();
  await api('/settings', 'PUT', { services: { codex: true, defaultEngine: 'codex' } });

  // Exact first approval: preview, then Go ahead in the dialog.
  await page.getByRole('region', { name: 'Needs your OK' }).getByRole('button', { name: 'Show me first', exact: true }).click();
  const preview = page.getByRole('dialog');
  await expect(preview).toBeVisible();
  await expect(preview.getByText(/Scope fixture first proposal/)).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath('scope-first-preview.png'), animations: 'disabled', fullPage: true });
  ...
  expect(await fs.readFile(path.join(project.folder, 'Autonomy-proof.md'), 'utf8')).toContain('Scope fixture first proposal');
  ...
  await page.screenshot({ path: testInfo.outputPath('permission-panel.png'), animations: 'disabled', fullPage: true });
  await fs.mkdir(EVIDENCE, { recursive: true });
  await page.screenshot({ path: path.join(EVIDENCE, 'autonomy-permission-panel.png'), animations: 'disabled', fullPage: true });
  await dialog.getByRole('button', { name: 'Confirm task scope' }).click();
  await expect(page.locator('.task-permission').getByRole('button', { name: 'Work in this project' })).toBeVisible({ timeout: 15_000 });
  await page.keyboard.press('Escape');
  const grants = await api<{ grants: { active: boolean; grant: { taskId: string } }[] }>(`/projects/${project.id}/permissions/grants`);
  ...
  expect(await fs.readFile(path.join(project.folder, 'Scope-note-A.md'), 'utf8')).toContain('Second-run scoped write');
  expect(await fs.readFile(path.join(project.folder, 'Scope-note-B.md'), 'utf8')).toContain('Second-run scoped write');
  await openThread(page, taskName);
  ...
  await expect(inspector.getByRole('heading', { name: 'Recorded authorization', exact: true })).toBeVisible();
  ...
  await expect(inspector.getByRole('heading', { name: 'Recorded sources', exact: true })).toBeVisible();
  await expect(inspector.getByRole('heading', { name: 'Runtime evidence', exact: true })).toBeVisible();
  await inspector.getByRole('button', { name: 'Refresh' }).click();
  await expect(inspector.getByText(/No detailed Runtime record is linked to this session/)).toBeVisible({ timeout: 15_000 });
  await page.screenshot({ path: testInfo.outputPath('scope-inspector.png'), animations: 'disabled', fullPage: true });
  await page.screenshot({ path: path.join(EVIDENCE, 'autonomy-scope-inspector.png'), animations: 'disabled', fullPage: true });

  // Board truth + Ledger recent history.
  await rail(page).getByRole('button', { name: /^Board/ }).click();
  await expect(page.locator('.board[aria-label="Board"]')).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath('scope-board.png'), animations: 'disabled', fullPage: true });
  await page.screenshot({ path: path.join(EVIDENCE, 'autonomy-scope-board.png'), animations: 'disabled', fullPage: true });
  state = await api<ProjectState>(`/projects/${project.id}/state`);
  expect(state.history.some((entry) => entry.kind === 'changed')).toBe(true);
  expect(errors).toEqual([]);
});

test('out-of-scope proposals wait for exact review; revocation forces exact approval again', async ({ page }, testInfo) => {
  test.setTimeout(90_000);
  const errors: string[] = [];
  page.on('pageerror', (error) => errors.push(error.message));
  ...
  const need = page.getByRole('region', { name: 'Needs your OK' });
  await expect(need).toBeVisible({ timeout: 20_000 });
  await expect(need.getByRole('button', { name: 'Allow creates and updates for this task' })).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath('boundary-prompt.png'), animations: 'disabled', fullPage: true });
  await page.screenshot({ path: path.join(EVIDENCE, 'autonomy-boundary-prompt.png'), animations: 'disabled', fullPage: true });
  ...
  expect(await fs.readFile(path.join(project.folder, 'Reopening plan.md'), 'utf8')).toBe(beforePlan);
  ...
  await preview.getByRole('button', { name: 'Go ahead', exact: true }).click();
  await expect(page.getByText('Approved changes applied. Their versions are in History.')).toBeVisible();
  expect(await fs.readFile(path.join(project.folder, 'Elsewhere.md'), 'utf8')).toContain('Outside the confirmed scope');
  ...
  await expect(page.locator('.task-permission').getByRole('button', { name: 'Review changes' })).toBeVisible({ timeout: 15_000 });
  await page.screenshot({ path: testInfo.outputPath('revoked-state.png'), animations: 'disabled', fullPage: true });
  await page.screenshot({ path: path.join(EVIDENCE, 'autonomy-revoked-state.png'), animations: 'disabled', fullPage: true });
  const grants = await api<{ grants: { active: boolean }[] }>(`/projects/${project.id}/permissions/grants`);
  ...
  await startFromBoard(page, taskName);
  await openThread(page, taskName);
  await expect(page.getByRole('region', { name: 'Needs your OK' })).toBeVisible({ timeout: 20_000 });
...
test('long names hold at 800px; Stop is truthful and inspector refresh stays honest', async ({ page }, testInfo) => {
  test.setTimeout(90_000);
  const errors: string[] = [];
  page.on('pageerror', (error) => errors.push(error.message));
  ...
  await page.screenshot({ path: testInfo.outputPath('overflow-800.png'), animations: 'disabled', fullPage: true });
  await page.screenshot({ path: path.join(EVIDENCE, 'autonomy-overflow-800.png'), animations: 'disabled', fullPage: true });
  ...
  await expect.poll(async () => (await api<ProjectState>(`/projects/${project.id}/state`)).sessions.at(-1)?.state).toBe('working');
  await expect(board.locator('.column[aria-label="Working"]').locator('.crow', { hasText: shortName.slice(0, 20) })).toBeVisible({ timeout: 15_000 });
  await page.screenshot({ path: testInfo.outputPath('board-working.png'), animations: 'disabled', fullPage: true });
  ...
  await expect(board.locator('.column[aria-label="Ready"]').locator('.crow', { hasText: shortName.slice(0, 20) })).toBeVisible({ timeout: 15_000 });
  ...
  await inspector.getByRole('button', { name: 'Refresh' }).click();
  await expect(inspector.getByText(/No detailed Runtime record is linked to this session/)).toBeVisible({ timeout: 15_000 });
  await page.screenshot({ path: testInfo.outputPath('inspector-refresh.png'), animations: 'disabled', fullPage: true });

  // Stale saved-approval evidence surfaces instead of silently clearing.
  await page.evaluate((id) => sessionStorage.setItem(`diomedes.approval.pending.${id}|broken`, '{broken'), project.id);
  await page.reload();
  await expect(page.getByRole('alert')).toContainText('A saved approval request could not be checked');
  await page.screenshot({ path: testInfo.outputPath('stale-evidence.png'), animations: 'disabled', fullPage: true });
  expect(errors).toEqual([]);
});
