...
describe('AWS conversation authority: independent integration counterexamples', () => {
  for (const mode of ['ask', 'plan'] as const) {
    test(`a real ${mode} message after work-input prevents new Work`, async () => {
      await connected();
      const commandId = `review-mode-${mode}`;
      const proposed = await send(commandId, 'ACT draft a linen checklist');
      if (proposed.outcome.status !== 'proposed') throw new Error('expected a proposal');
      const runtime = modelSessions();
      ...
      try {
        const response = await request(`${messages()}/${commandId}/select`, 'POST', {
        ...
        const tasks = current.tasks.filter((task) => task.creationReceipt?.commandId === ids.taskCommandId);
        const work = workFor(proposed.sourceMessageId);
  ...
  test('an acknowledged interrupted turn remains interrupted when polled before and after restart', async () => {
    await connected();
    let reached!: () => void;
    let waiting = false;
    const providerReached = new Promise<void>((resolve) => { reached = resolve; });
    // Deterministic barrier at the fake provider's actual request entry.
    Object.defineProperty(hang, 'waiting', {
      configurable: true,
      get: () => waiting,
      set: (value: boolean) => { waiting = value; if (value) reached(); },
    });
    const commandId = 'review-interrupted';
    const pending = send(commandId, 'SLOW await an interruption');
    await providerReached;
    const runId = lineageRun();
    const stopped = await api<{ acknowledged: boolean }>(`/projects/${project.id}/model-sessions/${runId}/interrupt`, 'POST', {
      commandId: 'review-stop',
    });
    expect(stopped.acknowledged).toBe(true);
    const completed = await pending;
    expect(completed.interrupted).toBe(true);
    const calls = seen.length;
    const phases = await modelSessions().phases(project.id, runId, completed.sourceMessageId);
    const beforeRestart = await api<MessageResult>(`${messages()}/${commandId}`);
    expect(await modelSessions().phases(project.id, runId, completed.sourceMessageId)).toEqual(phases);
    expect(seen).toHaveLength(calls);
    await close();
    await open();
    const recoveredPhases = await modelSessions().phases(project.id, runId, completed.sourceMessageId);
    const afterRestart = await api<MessageResult>(`${messages()}/${commandId}`);
    console.log('AWS_AUTHORITY_INTERRUPT', JSON.stringify({ post: completed, beforeRestart, afterRestart, calls: seen.length }));
    expect.soft(beforeRestart.interrupted).toBe(true);
    expect.soft(afterRestart.interrupted).toBe(true);
    expect(await modelSessions().phases(project.id, runId, completed.sourceMessageId)).toEqual(recoveredPhases);
    expect(seen).toHaveLength(calls);
    expect(workFor(completed.sourceMessageId)).toHaveLength(0);
  });

  test('a matching selection retry returns its existing receipt after source cancellation and narrowing', async () => {
    await connected();
    const commandId = 'review-receipt';
    const proposed = await send(commandId, 'ACT draft a linen checklist');
    if (proposed.outcome.status !== 'proposed') throw new Error('expected a proposal');
    const first = await select(commandId, proposed.outcome.proposalDigest);
    expect(first.outcome.status).toBe('started');
    await openNeed();
    await api(`/projects/${project.id}/harness/runs/${proposed.runId}/cancel`, 'POST', { reason: 'Independent review barrier' });
    await api(`/projects/${project.id}/threads/${thread.id}`, 'PUT', { mode: 'ask' });
    const phases = await modelSessions().phases(project.id, proposed.runId, proposed.sourceMessageId);
    const calls = seen.length;
    const before = workFor(proposed.sourceMessageId).map((work) => work.id);
    const response = await request(`${messages()}/${commandId}/select`, 'POST', {
      proposalDigest: proposed.outcome.proposalDigest,
      projectId: project.id,
      consent: true,
    });
    const body = await response.json() as MessageResult;
    console.log('AWS_AUTHORITY_RECEIPT', JSON.stringify({ status: response.status, first, replay: body, calls: seen.length }));
    expect.soft(response.status).toBe(200);
    expect.soft(body.outcome).toEqual(first.outcome);
    expect(await modelSessions().phases(project.id, proposed.runId, proposed.sourceMessageId)).toEqual(phases);
    expect(seen).toHaveLength(calls);
    expect(before).toHaveLength(1);
    expect(workFor(proposed.sourceMessageId).map((work) => work.id)).toEqual(before);
  });
});
