...
test('Connections works through ordinary Console navigation', async ({ page }) => {
  ...
  // Pause and resume stay on the Console. Connections is a Console-only surface
  // (AGENTS.md standing decision 1: the Workbook is frozen and takes no new screens),
  // so there is no second surface to switch to and no reload to survive.
  await page.setViewportSize({ width: 1280, height: 960 });
  await expect(page.getByRole('heading', { name: 'Reported availability' })).toBeVisible();
