...
describe('FD01 independent observed discovery and hostile output', () => {
  ...
  it('records the observed method when Windows managed fallback finds a verified installation', async () => {
    ...
    vi.spyOn(discoveryModule, 'createDiscovery').mockReturnValue({ discover: async () => absent });
...
async function packagingFixture() {
  const root = await temporaryRoot();
  for (const folder of ['client', 'server', 'shared', 'desktop', 'fixtures/harness', 'licenses', 'dist', 'scripts', 'node_modules/electron', '.data/native-runtime', 'cache'])
    await fs.mkdir(path.join(root, folder), { recursive: true });
  for (const file of ['desktop/main.mjs', 'desktop/app-updates.mjs', 'desktop/update-helper.mjs', 'desktop/diomedes.ico', 'desktop/service.ts', 'fixtures/harness/report-lines.txt', 'LICENSE', 'package-lock.json', 'scripts/package-desktop.mjs', 'dist/index.html'])
